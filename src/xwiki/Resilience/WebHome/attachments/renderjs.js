/*! RenderJs v0.2  */
/*global console, define, jQuery, localStorage, window, jIO */
...
;(function(scope) {
  ...
  var config = {
    ...
    // window.RENDERJS_ENABLE_IMPLICIT_GADGET_RENDERING
    implicitGadgetRendering: true,

    // by default RenderJs will examine and bind all interaction gadgets
    // available. This can be overridden by setting:
    // window.RENDERJS_ENABLE_IMPLICIT_INTERACTION_BIND
    implicitInteractionBind: true,

    // by default RenderJs will examine and create all routes
    // this can be overridden by setting
    // window.RENDERJS_ENABLE_IMPLICIT_ROUTE_CREATE
    implicitRouteCreate: true,
  };

  /*
   * How to use with require.js (AMD)
   *
   * Step 1: configure to your liking:
   *
   * requirejs.config({
   *   "path/to/renderjs": {
   *     implicitGadgetRendering: true,
   ...
   * });
   *
   * Step 2: require and recieve:
   *
   * require(["path/to/renderjs"], function(RenderJs) {
   *   ...and away you go!
   * });
   *
   */
  ...
  // map the configuration parameters so setting global variables works.
  for (var x in config) {
    config["_"+x] = config[x];
    config[x] = function () {
      var env = (function() {
        switch (x) {
          case "implicitRouteCreate":
            return "RENDERJS_ENABLE_IMPLICIT_GADGET_RENDERING";
          case "implicitInteractionBind":
            return "RENDERJS_ENABLE_IMPLICIT_GADGET_BIND";
          case "implicitRouteCreate":
            return "RENDERJS_ENABLE_IMPLICIT_ROUTE_CREATE";
          default: throw Error();
        }
      })();
      if (typeof(scope[env]) !== 'undefined') { return scope[env]; }
      return config["_"+x];
    }
  }

  var RenderJs;
  var RjsFunction = function (config, $, jIO) {
    ...
    return {
        ...
        init: function () {
           ...
           */
          if (config.implicitGadgetRendering()) {
              RenderJs.bootstrap($('body'));
          }
          var root_gadget = RenderJs.GadgetIndex.getRootGadget();
          if (config.implicitInteractionBind() || config.implicitRouteCreate()) {
  ...
  var createPrototypes = function (RequireJs) {
  ...
  if (typeof(define) === 'function' && define.amd) {
    // They have AMD so we'll define as an AMD module.
    define(['module', 'jquery', 'jio'], function(mod, jQuery, jIO) {
      var conf = mod.config() || {};
      for (var k in config) {
    ...
    // No AMD :( put RenderJs in the global scope and hope jQuery and jIO exist.
    scope.RenderJs = RenderJs = RjsFunction(config, jQuery, jIO);
    createPrototypes(RenderJs);
  }
